...
/**
 * Class ModuleGalleryList 
 *
 * @copyright  Thyon Design 2008 
 * @author     John Brand <john.brand@thyon.com> 
 * @package    Controller
 */
class ModuleGalleryList extends ModuleGallery
{
	/**
	 * Template
	 * @var string
	 */
	protected $strTemplate = 'mod_gallerylist';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### GALLERY LIST ###';
		...
		$objArchive = $this->Database->prepare("SELECT * FROM tl_gallery_archive WHERE id IN (" . implode(',', array_map('intval', $this->gallery_archives)) . ")")
																->execute();
																
		$this->arrArchives = $objArchive->numRows ? $objArchive->fetchAllAssoc() : array();

		// setup default sort mode (if unset)
		if (!$this->gallery_sort)
		{
			$this->gallery_sort = 'date';
		}
